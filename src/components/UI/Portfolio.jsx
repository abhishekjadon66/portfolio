...
const Portfolio = () => {
    ...
    useEffect(() => {
        ...
        if (selectTab === 'web-design') {
            const filterData = portfolio.filter(item => item.category === 'Web Design')
        ...
        <section id="portfolio">
            <div className="container">
                ...
                <div className="flex items-center gap-4 flex-wrap mt-12">
                    {portfolios?.slice(0, nextItems)?.map((portfolios, index) => (
                        <div
                            ...
                            className="group max-w-full sm:w-[31.8%] relative z-[1]"
                        >
                            <figure>
                            ...
                            <div className="w-full h-full bg-primaryColor bg-opacity-40 absolute top-0 left-0 z-[5] hidden group-hover:block rounded-[8px]">
                                <div className="w-full h-full flex items-center justify-center">
                                    <button onClick={() => showModalHandler(portfolios.id)} className="text-white bg-headingColor hover:bg-smallTextColor py-2 px-4 rounded-[8px] font-[500] ease-in duration-200">
                                        See Details
                                    </button>
                                </div>
                            </div>
                ...
                <div className="text-center mt-6">
                    {nextItems < portfolios.length && portfolios.length > 6 && <button onClick={loadMoreHandler} className="text-white bg-primaryColor hover:bg-smallTextColor py-2 px-4 rounded-[8px] font-[500] ease-in duration-200">
                        Load More
                    </button>}
